...
import { getPackageManager } from "../lib/getPackageManager.js";
...
.name("init")
.description("initialize Aura UI into your project")
// .argument // Not supporting to add components while initializing aura-ui. If needed use this to optionally accept the components to add
.action(async () => {
  const cwd = process.cwd();
  const configFilePath = path.join(cwd, "aura-ui.config.json");

  const spinner = getSpinner("Initializing Aura UI");
  ...
  try {
    ...
    spinner.succeed("Aura UI initialized successfully.");
    console.log("You can now run `npx aura-ui add componentName` to add components to your project");
  } catch(error) {
    spinner.fail("Error while initializing Aura UI to your project.");
